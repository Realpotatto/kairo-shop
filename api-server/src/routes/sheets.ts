...
router.get("/sheets/:name", requireAuth, async (req: any, res) => {
  try {
    ...
    if (err.message?.includes("not set")) {
      ...
      req.log.error({ err }, "sheets GET error");
...
router.put("/sheets/:name", requireAdmin, async (req: any, res) => {
  try {
    const sheetId = getSheetId(req.params.name);
    ...
    if (err.message?.includes("not set")) {
      ...
      req.log.error({ err }, "sheets PUT error");
...
router.post("/sheets/:name/append", requireAdmin, async (req: any, res) => {
  try {
    const sheetId = getSheetId(req.params.name);
    const { range = "Sheet1", values, objects } = req.body;
    ...
    if (err.message?.includes("not set")) {
      ...
      req.log.error({ err }, "sheets POST error");
...
router.delete("/sheets/:name", requireAdmin, async (req: any, res) => {
  try {
    const sheetId = getSheetId(req.params.name);
    ...
    if (err.message?.includes("not set")) {
      ...
      req.log.error({ err }, "sheets DELETE error");
