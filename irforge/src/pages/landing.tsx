...
import { Terminal, Blocks, Zap, ChevronRight, Github, Shield, BarChart3, Bot, Sun, Moon, Languages } from "lucide-react";
        ...
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="relative inline-flex items-center gap-2 rounded-full border border-border bg-background px-3 py-1.5 text-xs font-medium text-muted-foreground transition-all hover:border-primary hover:text-primary hover:bg-primary/5 group"
              aria-label={tr.toggleTheme}
            >
              <span className="relative size-4 flex items-center justify-center overflow-hidden">
                <Sun
                  className={`absolute size-3.5 transition-all duration-300 ${isDark ? "opacity-100 rotate-0 scale-100" : "opacity-0 -rotate-90 scale-50"}`}
                />
                <Moon
                  className={`absolute size-3.5 transition-all duration-300 ${!isDark ? "opacity-100 rotate-0 scale-100" : "opacity-0 rotate-90 scale-50"}`}
                />
              </span>
              <span className="hidden sm:inline">{tr.toggleTheme}</span>
            </button>

            {/* Language Toggle Button */}
            <button
              onClick={toggleLang}
              className="inline-flex items-center gap-1.5 rounded-full border border-border bg-background px-3 py-1.5 text-xs font-medium text-muted-foreground transition-all hover:border-primary hover:text-primary hover:bg-primary/5"
              aria-label="Toggle language"
            >
              <Languages className="size-3.5" />
              <span>{tr.toggleLang}</span>
            </button>
            ...
            <Button asChild size="sm">
              <Link href={user ? "/dashboard" : "/register"}>
                {user ? tr.dashboard : tr.getStarted}
              </Link>
            </Button>
          </div>
        </div>
        ...
        <section className="py-20 md:py-32 border-b relative overflow-hidden">
          ...
          <div className="container mx-auto px-4 relative z-10 text-center max-w-5xl">
            ...
            <h1 className="text-5xl md:text-7xl font-extrabold tracking-tight mb-6 leading-tight">
              {lang === "fa" ? (
                <>
                  پلتفرم حرفه‌ای<br className="hidden md:inline" />
